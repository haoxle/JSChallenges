...
export const getIsValidOnNameTag = () => {
  if (
    ...
    nameTagOption.substring(0, 1) ===
      nameTagOption.substring(0, 1).toUpperCase()
  ) {
    return true;
  } else {
    return false;
...
export const getHasUppercaseLetters = () => {
  if (
    stringWithUppercaseLetters.substring(0, -1) ===
    stringWithUppercaseLetters.substring(0, -1).toUpperCase()
  ) {
    return true;
  } else {
    return false;
...
export const convertPascalCaseToSnakeCase = (str) =>  str.split(/(?=[A-Z])/).join("_").toLowerCase();